...
export function CleaningResultStats({ report }: { report: CleaningReport }) {
  ...
  const items: { label: string; value: string }[] = [
    ...
    { label: "Duplicates Removed", value: s.duplicatesRemoved.toLocaleString() },
    ...
    <Card>
      <CardHeader>
        <div>
          ...
          <CardDescription>Generated automatically after the pipeline finished</CardDescription>
        </div>
      ...
      <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {report.operations.map((op) => (
              <div key={op.label} className="flex items-center gap-2 text-sm">
                ...
                <span className={op.done ? "text-ink-100" : "text-ink-500"}>{op.label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {report.columnConversions.length > 0 && (
          <div>
            <p className="text-xs font-medium text-ink-300 mb-2">Column type conversions</p>
        ...
        <div>
          <p className="text-xs font-medium text-ink-300 mb-2">Recommendations</p>
        ...
        <div className="rounded-xl border border-signal-good/25 bg-signal-good/[0.06] px-4 py-3 flex items-center gap-2.5">
          <CheckCircle2 className="h-4 w-4 text-signal-good shrink-0" />
          <p className="text-sm text-signal-good font-medium">Data ready for analysis</p>
        </div>
      </div>
    </Card>
  );
